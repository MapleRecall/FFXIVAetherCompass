...
internal static class ZoneWatcher
{
	...
	public static Excel.Map? CurrentMap
	{
		get
		{
			if (cachedMap == null || CurrentMapId != cachedMap.Value.RowId)
				cachedMap = GetMap();
			return cachedMap;
		}
	}
	...
	private static void CheckCompassWorkZone()
	{
		IsInCompassWorkZone =
			...
			&& CurrentTerritoryType.Value.TerritoryIntendedUse.ValueNullable?.RowId
				!= 20 // chocobo race terr?
		;
	}
	...
	private static void CheckDetailWindowHideZone()
	{
		...
		IsInDetailWindowHideZone =
			CurrentTerritoryType == null || CurrentTerritoryType.Value.ExclusiveType > 0;
	}
}
